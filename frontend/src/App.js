...
const IS_MAINTENANCE = false; // Toggle for protocol upgrades
  ...
  return (
    <div style={{ backgroundColor: theme.bg, minHeight: "100vh", color: theme.textMain, fontFamily: "'Inter', sans-serif" }}>
          ...
          <>
            {!userData ? (
              <div style={{ textAlign: "center", paddingTop: "100px" }}>
                <div style={badge}>V{APP_VERSION} LIVE</div>
                <h1 style={{ fontSize: "60px", fontWeight: "900" }}>Diamond Hands <br/><span style={{ color: theme.primary }}>As A Service.</span></h1>
                <p style={{ color: theme.textMuted, marginBottom: "30px" }}>Secure time-locked savings on the Stacks blockchain.</p>
                <button onClick={() => showConnect({ userSession, appDetails: { name: "STX Vault" }, onFinish: () => window.location.reload() })} style={heroBtn}>Get Started</button>
              </div>
            ) : (
              <div style={gridContainer}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                  <div style={cardStyle}>
                    ...
                    {history.map(tx => (
                      <a key={tx.tx_id} href={`https://explorer.hiro.so/txid/${tx.tx_id}`} target="_blank" style={historyRow}>
                        <span>{tx.contract_call.function_name.replace("-stx","")}</span>
                        <span style={{color:theme.success}}>Confirmed</span>
                      </a>
                    ))}
                  </div>
                  <div style={cardStyle}>
      ...
      <footer style={footerStyle}>
        <div style={{ display: "flex", gap: "20px", color: theme.textMuted, fontSize: "12px" }}>
          <span onClick={()=>setShowLegal(true)} style={{cursor:"pointer"}}>Terms</span>
        ...
        {showConfirm && (
          <div style={modalOverlay}>
            <motion.div initial={{scale:0.9}} animate={{scale:1}} style={modalContent}>
              ...
              <p>This triggers a 10% penalty fee. Continue?</p>
...
const inputStyle = { width: "100%", padding: "12px", marginBottom: "10px", borderRadius: "10px", border: `1px solid ${theme.cardBorder}`, backgroundColor: "#000", color: "#fff", outline: "none" };
const actionBtn = { width: "100%", padding: "14px", borderRadius: "10px", backgroundColor: theme.primary, color: "#fff", border: "none", fontWeight: "800", cursor: "pointer" };
const connectBtn = { backgroundColor: theme.primary, color: "#fff", border: "none", padding: "10px 20px", borderRadius: "10px", fontWeight: "700" };
const exitBtn = { backgroundColor: "rgba(239,68,68,0.1)", border: `1px solid ${theme.danger}44`, color: theme.danger, padding: "8px", borderRadius: "10px" };
...
const badge = { color: theme.primary, border: `1px solid ${theme.primary}`, padding: "4px 12px", borderRadius: "100px", fontSize: "10px", fontWeight: "900", marginBottom: "20px", display: "inline-block" };
const heroBtn = { backgroundColor: theme.primary, color: "#fff", padding: "16px 40px", borderRadius: "14px", fontSize: "18px", fontWeight: "800", border: "none" };
const shareBtn = { width: "100%", padding: "10px", marginTop: "15px", borderRadius: "10px", backgroundColor: "#000", border: `1px solid ${theme.cardBorder}`, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", fontSize: "13px", fontWeight: "700" };
...
const cardHead = { display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px", fontSize: "16px" };
const statRow = { display: "flex", alignItems: "center", gap: "12px", marginBottom: "10px" };
const statValue = { fontSize: "20px", fontWeight: "800" };
const historyRow = { display: "flex", justifyContent: "space-between", padding: "12px", backgroundColor: "rgba(255,255,255,0.02)", borderRadius: "10px", marginBottom: "8px", textDecoration: "none", color: "inherit", fontSize: "13px" };
const leaderRow = { display: "flex", justifyContent: "space-between", padding: "10px", borderBottom: `1px solid ${theme.cardBorder}`, fontSize: "13px" };
...
const modalContent = { backgroundColor: theme.card, padding: "40px", borderRadius: "30px", textAlign: "center", maxWidth: "400px" };
const confirmBtn = { padding: "12px 20px", backgroundColor: theme.danger, color: "#fff", border: "none", borderRadius: "10px", fontWeight: "700" };
const secondaryBtn = { padding: "12px 20px", backgroundColor: "#30363D", color: "#fff", border: "none", borderRadius: "10px", fontWeight: "700" };
const dangerBtn = { backgroundColor: "transparent", color: theme.danger, border: `1px solid ${theme.danger}`, padding: "10px", borderRadius: "10px", fontSize: "12px", fontWeight: "700" };
const maintCard = { textAlign: "center", padding: "100px", backgroundColor: theme.card, borderRadius: "30px", border: `1px solid ${theme.cardBorder}` };
