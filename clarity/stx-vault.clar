...
(define-public (wthdraw-stx)
    (let (
        (user-deposit (map-get? deposits { owner: tx-sender, unlock-block: (get unlock-block (map-get? deposits { owner: tx-sender, unlock-block: (get unlock-block (mat? deposs { oner: tx-sender, unlock-block: u0 }))}) }))
