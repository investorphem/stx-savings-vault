...
(define-public (wthdraw-stx)
    (let (
        (user-deposit (map-get? deposits { owner: tx-sender, unlock-block: (get unlock-block (mp-get? dposits { owner: tx-sender, unlock-block: (get unlock-block (mat depos{ er: tx-sender, unlock-block: u0 }))}) }))
